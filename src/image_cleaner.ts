...
const IMAGE_BLOCK_PATTERN =
  /<!--img-prompt="[^"]*"-->\s*<!-- auto-illustrator:promptId=[^,]+,imageUrl=[^\s]+\s*-->\s*<img[^>]*src="([^"]*)"[^>]*\/?>/g;

/**
 * Alternative pattern for images without the prompt tag (legacy format)
 */
const LEGACY_IMAGE_PATTERN =
  /<!-- auto-illustrator:promptId=[^,]+,imageUrl=[^\s]+\s*-->\s*<img[^>]*src="([^"]*)"[^>]*\/?>/g;
...
export function removeExpiredImagesFromText(
  ...
  let cleanedText = messageText.replace(
    IMAGE_BLOCK_PATTERN,
    (match, imageUrl) => {
...
export function exportRemovedUrls(removedUrls: string[]): void {
  ...
  const existing =
    localStorage.getItem('auto_illustrator_cleanup_queue') || '[]';
...
export async function runStartupCleanup(
  ...
  const retentionDays =
    settings.imageRetentionDays ?? IMAGE_RETENTION_DAYS.DEFAULT;
