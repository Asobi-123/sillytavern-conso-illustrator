...
如果你在酒馆 **Image Generation** 扩展里保存了多套 Style 组合（比如不同画师串、不同负面前缀），平时只能手动切换。

在插件 **主控台 → 每次生图随机抽取 SD Style** 卡片里打开开关后，conso 会在每次生图前从酒馆已保存的 Style 列表中**随机抽一条**临时套用，生图后还原原值。可以让一段对话里出来的图自然在多种风格之间切换，不用你手动改。

- **白名单**：列表里勾选的 Style 才会参与抽签；全不勾 = 使用全部
...
安装后端插件后，打开 conso 悬浮面板，在 **Vibe Transfer** 卡片里启用功能并上传参考图。
...
Tag 超市是插件内置的离线 tag catalog，不需要运行时联网，也不需要用户自己收集基础 tag。当前内置 catalog 版本是 `2026-06`，共 7928 个 tag。
...
*本教程适用于 Conso Illustrator v1.12.0+*
