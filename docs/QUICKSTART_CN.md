...
- **编码 Vibe**：从 `.naiv4vibebundle.json` 导入，只有编码也能用于生图，不需要源图。
...
- 导入 bundle 时不会覆盖已有 Vibe。若外部 ID 冲突，Conso 会创建新的本地 ID，并保留原外部 ID 用于导出。
- 导出 bundle 只包含编码 Vibe 的标准字段，不包含本地源图、预览图和本地搜索标签。只有源图但还没有编码的项目会被跳过。
- `.naiv4vibebundle.json` 兼容当前观察到的 NovelAI Vibe bundle 形状；如果其他工具扩展了私有字段，Conso 会只保留可识别的编码数据。
...
Vibe 组合用于保存“启用了哪些 Vibe 和它们的参数”。默认按钮是 **另存为**，不会覆盖现有组合；需要覆盖时使用单独的 **覆盖组合** 按钮，并确认目标组合名称。
